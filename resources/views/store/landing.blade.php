        ...
        <div class="container-fluid fixed-top">
            ...
            <div class="container px-0">
                <nav class="navbar navbar-light bg-white navbar-expand-xl">
                    <a href="index.html" class="navbar-brand"><h1 class="text-primary display-6">{{ $setting->business_name }}</h1></a>
                                    ...
                                    <div class="row g-4">
                                        @forelse($products as $product)
                                            <div class="col-md-6 col-lg-4 col-xl-3 d-flex">
                                                {{-- Move border classes to fruite-item --}}
                                                <div class="rounded position-relative fruite-item w-100 border border-secondary"> 
                                                    <div class="fruite-img product-card-img-container rounded-top"> {{-- Add rounded-top here for image --}}
                                                        @if (!empty($product->product_img))
                                                            <img src="{{ asset('storage/' . $product->product_img) }}" class="img-fluid w-100 rounded-top" alt="{{ $product->name }}">
                                                        @else
                                                            <img src="{{ $placeholderProductImg ?? asset('build/assets/frontend/img/default-product.jpg') }}" class="img-fluid w-100 rounded-top" alt="No Image">
                                                        @endif
                                                    </div>
                                                    <div class="text-white bg-secondary px-3 py-1 rounded position-absolute" style="top: 10px; left: 10px;">
                                                        {{ $product->category->name ?? 'Uncategorized' }}
                                                    </div>
                                                    {{-- Remove border-top-0 and rounded-bottom from here --}}
                                                    <div class="p-4 d-flex flex-column"> 
                                                        <h4 style="font-size: 16px;">{{ Str::words($product->name, 5, '...') }}</h4>
                                                        <p class="flex-grow-1">{{ Str::limit($product->description ?? 'No description available.', 70) }}</p>
                                                        <div class="d-flex justify-content-between flex-lg-wrap mt-auto">
                                                            <p class="text-dark fs-5 fw-bold mb-0" style="font-size: 10px;">{{ $setting->currency_symbol }} {{ number_format($product->sale_price ?? 0, 2) }} / {{ $product->displayUnit->name ?? 'unit' }}</p>
