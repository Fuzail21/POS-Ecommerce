<div id="invoice-content" class="font-mono text-sm text-gray-800 leading-tight">
    <div class="text-center mb-2">
        <h2 class="text-2xl font-extrabold tracking-wide text-gray-900">POS INVOICE</h2>
    ...
    <div class="space-y-1 mb-3">
        @foreach($sale->items as $item)
            <div class="flex justify-between">
                ...
                <div class="w-1/5 text-right">${{ number_format($item->total_price, 2) }}</div>
            </div>
        @endforeach
    </div>
    ...
    <div class="space-y-1 text-sm">
        <div class="flex justify-between"><span>Subtotal:</span><span>${{ number_format($sale->total_amount, 2) }}</span></div>
        <div class="flex justify-between"><span>Discount:</span><span>${{ number_format($sale->discount_amount, 2) }}</span></div>
        <div class="flex justify-between"><span>Tax:</span><span>${{ number_format($sale->tax_amount, 2) }}</span></div>
        <div class="flex justify-between"><span>Shipping:</span><span>${{ number_format($sale->shipping, 2) }}</span></div>
        <hr class="border-t border-gray-300 my-1">
        <div class="flex justify-between font-bold text-base text-gray-900">
            <span>Grand Total:</span><span>${{ number_format($sale->final_amount, 2) }}</span>
        </div>
    </div>
